...
def show_menu():
    ...
    # Button to navigate to U-Substitution quiz
    if st.button("U-Substitution (Practice)"):
        set_page("u_sub")
    # Button to navigate to Integration by Parts (IBP) quiz
    if st.button("Integration by Parts"):
        set_page("ibp")
    # Button to navigate to Trigonometric Integrals quiz
    if st.button("Trigonometric Integrals"):
        set_page("trig")
...
def show_quiz():
    ...
    if st.session_state.user_input:
        st.write("Your input:")
        # One liner madness
        st.latex(re.sub(r'(operatorname{a)(sin|cos|tan|csc|sec|cot)(h?)', r'\2\3^{-1', sp.latex(st.session_state.user_input)))
...
# Start the app by running the main function
